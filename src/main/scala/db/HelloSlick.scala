...
object HelloSlick extends App {
  ...
  println("Nested collections are not supported, so we need to flatten the nested queries by aggregating values")
  val grouped2 = grouped.map { case (supID, css) =>
    (supID, css.length, css.map(_._1.price).avg)
  }
  println("Generated sql: " + grouped2.selectStatement)
    println(grouped2.run)

  println("Workaround : Produce the nested results locally instead of as a query")
  val grouped3 = (for {
    c <- coffees
    s <- c.supplier
  } yield (c, s)).list.groupBy( _._2._1) // list materialises the result - _._2._1 is s.id
